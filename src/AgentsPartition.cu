...
AgentsPartition::AgentsPartition(int handle, void *argument, int argument_size,
...
void AgentsPartition::updateRightGhost(void *ghost, cudaStream_t stream) {
	...
	if (rank < numRanks - 1) {
		if (isloaded) {
			cudaMemcpyAsync(movePtr(dPtr, numElements), ghost,
					Tbytes * ghostWidth, cudaMemcpyHostToDevice, stream);
		} else {
